...
bool optimized_water_values__deterministic__percentage_of_spot (int curr_day, bool include_reserve, tResultsFile &results) {
    ...
    // calculate water values
    solve_primal_master_problem__deterministic__percentage_of_spot (include_reserve, &results.daily_records[curr_day].water_values);

    // solve problem for the first hour of the day
    vector<double> bid__spot, bid__reserve_up, bid__reserve_down;
    solve_first_hour_of_daily_problem__deterministic__percentage_of_spot (results.daily_records[curr_day].water_values,
                                                      include_reserve,
                                                      &bid__spot, &bid__reserve_up, &bid__reserve_down);
    ...
    spill.resize (no_of_arcs * no_of_hours_per_day, 0.0);
    for (int theta = 0; theta < no_of_hours_per_day; ++theta) {
        double v = solve_each_hour_of_daily_problem__deterministic__percentage_of_spot (theta, results.daily_records[curr_day].water_values,
            bid__spot, bid__reserve_up, bid__reserve_down,
            generate, pump, spill);
...
int main (int argc, const char *argv[], const char *param_env[]) {
    ...
    // read deterministic__percentage_of_spot__factor to work on
    if (argc != 2)
        error ("main", "Expected single command line argument with deterministic__percentage_of_spot__factor.");
    
    int v = atoi (argv[1]);
    switch (v) {
        case 200:   deterministic__percentage_of_spot__factor = 2.0; break;
        case 300:   deterministic__percentage_of_spot__factor = 3.0; break;
        case 400:   deterministic__percentage_of_spot__factor = 4.0; break;
        case 500:   deterministic__percentage_of_spot__factor = 5.0; break;
        default:    error ("main", "Incorrect deterministic__percentage_of_spot__factor"); break;
    }
    ...
    filename << directory << "/Results/det_" << v << "_" << curr_out_of_sample_no << ".txt";
