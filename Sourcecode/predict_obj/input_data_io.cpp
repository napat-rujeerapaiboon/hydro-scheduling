...
void read_data (const char *curr_dir) {
    ...
    for (int s = 0; s < no_of_insample_scenarios__data; ++s) {
        ...
        stringstream filename;
        filename << curr_dir << "/hourly_spot_prices.insample_" << s << ".txt";
        ifstream fin_spot (filename.str().c_str());
        if (!fin_spot.is_open()) error ("read_data", "Could not open data file.");

        filename.str (string());
        filename << curr_dir << "/hourly_reserve_up_prices.insample_" << s << ".txt";
        ifstream fin_up (filename.str().c_str());
        if (!fin_up.is_open()) error ("read_data", "Could not open data file.");

        filename.str (string());
        filename << curr_dir << "/hourly_reserve_down_prices.insample_" << s << ".txt";
        ifstream fin_down (filename.str().c_str());
        if (!fin_down.is_open()) error ("read_data", "Could not open data file.");
    ...
    {
        stringstream filename;
        filename << curr_dir << "/hourly_spot_prices.outofsample.txt";
        ifstream fin_spot (filename.str().c_str());
        if (!fin_spot.is_open()) error ("read_data", "Could not open data file.");

        filename.str (string());
        filename << curr_dir << "/hourly_reserve_up_prices.outofsample.txt";
        ifstream fin_up (filename.str().c_str());
        if (!fin_up.is_open()) error ("read_data", "Could not open data file.");

        filename.str (string());
        filename << curr_dir << "/hourly_reserve_down_prices.outofsample.txt";
        ifstream fin_down (filename.str().c_str());
        if (!fin_down.is_open()) error ("read_data", "Could not open data file.");
    ...
    {
        stringstream filename;
        filename << curr_dir << "/hourly_reserve_up_calls.outofsample.txt";
        ifstream fin_up (filename.str().c_str());
        if (!fin_up.is_open()) error ("read_data", "Could not open data file.");

        filename.str (string());
        filename << curr_dir << "/hourly_reserve_down_calls.outofsample.txt";
        ifstream fin_down (filename.str().c_str());
        if (!fin_down.is_open()) error ("read_data", "Could not open data file.");
    ...
    for (int s = 0; s < no_of_insample_scenarios__data; ++s) {
        stringstream filename;
        filename << curr_dir << "/hourly_inflows.insample_" << s << ".txt";
        ifstream fin (filename.str().c_str());
        if (!fin.is_open()) error ("read_data", "Could not open data file.");
    ...
    {
        stringstream filename;
        filename << curr_dir << "/hourly_inflows.outofsample.txt";
        ifstream fin (filename.str().c_str());
        if (!fin.is_open()) error ("read_data", "Could not open data file.");
