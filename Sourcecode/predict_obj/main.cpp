...
void optimized_water_values (int curr_day, bool include_reserve, tResultsFile &results, const char *filename) {
    ...
    double objval = solve_primal_master_problem__stochastic (include_reserve, &results.daily_records[curr_day].water_values, filename);
...
int main (int argc, const char *argv[], const char *param_env[]) {
    ...
    // set #-of-samples to work on
    no_of_insample_scenarios__model = 10;
    ...
    filename << directory << "/Results/predict_" << curr_out_of_sample_no << ".txt";
    ...
    // run the model
    filename.str (string());
    filename << directory << "/Results/prediction_obj_" << curr_out_of_sample_no << ".txt";
    optimized_water_values (curr_day, true, results, filename.str().c_str());
