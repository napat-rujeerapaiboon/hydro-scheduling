...
int main (int argc, const char *argv[], const char *param_env[]) {
    ...
    filename << directory << "/Results/nosamples_I_do_not_exist.txt";
    ...
    // run the model
    optimized_water_values (curr_day, true, results);
    
    // write results files
    filename.str (string());
    filename << directory << "/Results/runtimes_" << no_of_insample_scenarios__model << "_" << curr_out_of_sample_no << ".txt";
    cout << "writing to \"" << filename.str().c_str() << "\"." << endl;
    ofstream fout (filename.str().c_str());
    fout << runtime_master << endl;
    fout << runtime_slave << endl;
