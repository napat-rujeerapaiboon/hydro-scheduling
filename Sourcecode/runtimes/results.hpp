...
/*
   ...
   *******************************
   ...
   2) initial reservoir levels
 
   3) master problem optimal objective value
 
   4) water values for the current day

   5) for each hour of the current day:

       5)d)1) spot bid for that hour, spot revenues for that hour
       5)d)2) reserve-up bid for that hour, reserve-up revenues for that hour
       5)d)3) reserve-down bid for that hour, reserve-down revenues for that hour
       5)d)4) missed reserve-up commitment, missed reserve-down commitment for that hour
       5)d)5) generate (for all arcs), pump (for all arcs), spill (for all arcs)

   6) terminal reservoir levels
*/
...
struct tResultsFile {
    std::vector<tResults_DailyRecord>   daily_records;                      // [no_of_days_per_year]
